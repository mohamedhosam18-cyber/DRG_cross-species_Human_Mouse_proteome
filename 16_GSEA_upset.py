...
fig_dir = base / "figures" / "gsea_upset"
fig_dir.mkdir(parents=True, exist_ok=True)

datasets = [
    {
        "name": "GO_BP",
        "folder": results_dir / "GSEA_GO_BP",
        "human_out": fig_dir / "GSEA_GO_BP_human_leading_edge_custom_upset.png",
        "mouse_out": fig_dir / "GSEA_GO_BP_mouse_leading_edge_custom_upset.png",
    },
    {
        "name": "KEGG",
        "folder": results_dir / "GSEA_KEGG",
        "human_out": fig_dir / "GSEA_KEGG_human_leading_edge_custom_upset.png",
        "mouse_out": fig_dir / "GSEA_KEGG_mouse_leading_edge_custom_upset.png",
    },
    {
        "name": "Reactome",
        "folder": results_dir / "GSEA_Reactome",
        "human_out": fig_dir / "GSEA_Reactome_human_leading_edge_custom_upset.png",
        "mouse_out": fig_dir / "GSEA_Reactome_mouse_leading_edge_custom_upset.png",
    },
    {
        "name": "WikiPathways",
        "folder": results_dir / "GSEA_WikiPathways",
        "human_out": fig_dir / "GSEA_WikiPathways_human_leading_edge_custom_upset.png",
        "mouse_out": fig_dir / "GSEA_WikiPathways_mouse_leading_edge_custom_upset.png",
    },
]
...
def prepare_subset(df, direction="human"):
    ...
    if direction == "human":
        df = df[df["NES"] > 0].sort_values(["FDR", "NES"], ascending=[True, False]).head(TOP_N).copy()
    else:
        df = df[df["NES"] < 0].sort_values(["FDR", "NES"], ascending=[True, True]).head(TOP_N).copy()
...
for ds in datasets:
    print(f"\n===== {ds['name']} =====")
    report_file = find_report_file(ds["folder"])
    ...
    print("\nTop human-enriched pathways used:")
    print(human_df[["Term", "NES", "FDR"]])

    make_custom_upset(
        human_df,
        f"Leading-edge gene overlap across top human-enriched {ds['name']} pathways",
        ds["human_out"],
        color="#cc6f12"
    )
    ...
    print("\nTop mouse-enriched pathways used:")
    print(mouse_df[["Term", "NES", "FDR"]])

    make_custom_upset(
        mouse_df,
        f"Leading-edge gene overlap across top mouse-enriched {ds['name']} pathways",
        ds["mouse_out"],
        color="#2f8a57"
    )
...
print("\nAll GO BP + KEGG + Reactome + WikiPathways GSEA UpSet plots completed.")
